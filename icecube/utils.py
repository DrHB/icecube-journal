...
def filter_ds_based_on_kappa(ds, bs, kappa = 0.5, lenth=196):
    df = pd.read_csv(f'/opt/slh/icecube/data/SUB/submission_kappas/EXP_25_FT_{bs}_train.csv')
    df.columns = ['event_id', 'pulse_duration', 'divkappa']
    event_ids = df.query('divkappa > @kappa').query("pulse_duration < @lenth").index.values
    ds = ds.select(event_ids)
    return ds
...
def fit_shuflle(
    ...
    if config.FILTER:
        vld_pth = [
            filter_ds_based_on_kappa(load_from_disk(config.DATA_CACHE_DIR / f"batch_{i}.parquet"), i)
    ...
    for i in mb:  # iterating  epoch
        ...
        if config.FILTER:
            trn_pth = [
                filter_ds_based_on_kappa(load_from_disk(config.DATA_CACHE_DIR / f"batch_{i}.parquet"), i) for i in nums
            ]
...
def gget_score_vector(y_hat, y, return_df=False):
    y_hat = label_to_df(y_hat.detach().cpu().numpy()[:, :3])
...
def gget_score_save(y_hat, y, return_df=False):
    y_hat = label_to_df(y_hat.detach().cpu().numpy()[:, :3])
    y = label_to_df(y.reshape(-1, 3).detach().cpu().numpy())
    score = angular_dist_score(
            torch.tensor(y["azimuth"].values, dtype=torch.float32),
...
def eval_save(
    ...
    with torch.no_grad():
        for batch in tqdm(valid_dl):
            ...
            pred.append(out.detach())
    # calculating metric
    metric_, gt_df, pd_df = metric(torch.cat(pred), torch.cat(gt), return_df=True)
    print(f"metric: {metric_}")

    gt_df.to_csv(f"{Path(folder)/exp_name}_gt.csv", index=False)
    pd_df.to_csv(f"{Path(folder)/exp_name}_pd.csv", index=False)
